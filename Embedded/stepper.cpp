...
uint32_t thread_period = 121; // Thread period in ms
...
Telecommandmsg TelecommandDataReceiverST; //Input

CommBuffer<Telecommandmsg> TelecommandDataBufferST;
Subscriber TelecommandDataSubscriber3(TelecommandDataTopic, TelecommandDataBufferST);
...
sPID_Data PID_StepperSpeed = {1, 0.5, 0,                               // float Kp, Ki, Kd;
		...
		6, -6, (float)thread_period/1000,     // float Umax, Umin, T;
		1};                                       // bool AntiWindup;
...
public:
	...
	Balancer_Thread(const char* name) : Thread(name) {
	}
	...
	void run () {
		...
		TIME_LOOP(0,  thread_period){
			...
			if(TelecommandDataBufferST.getOnlyIfNewData(TelecommandDataReceiverST)){

				if(TelecommandDataReceiverST.SystemMode == 3){
					set_stepper_speed((int32_t)TelecommandDataReceiverST.STSpeed[0], 1);
					set_stepper_speed((int32_t)TelecommandDataReceiverST.STSpeed[1], 2);
					set_stepper_speed((int32_t)TelecommandDataReceiverST.STSpeed[2], 3);
				}else if(TelecommandDataReceiverST.SystemMode == 7){
					set_stepper_speed((int32_t)TelecommandDataReceiverST.STSpeed[0], 1);
					set_stepper_speed((int32_t)TelecommandDataReceiverST.STSpeed[1], 2);
					set_stepper_speed((int32_t)TelecommandDataReceiverST.STSpeed[2], 3);
				}
			}
			thread_period = TelecommandDataReceiverST.ThreadPeriod[5] <= -1? thread_period:TelecommandDataReceiverST.ThreadPeriod[5];
		}
	}
};

Balancer_Thread balancer_thread("Balancer");
