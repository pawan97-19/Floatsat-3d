...
struct Telecommandmsg{
	...
	int ThreadPeriod[5]; // in Milliseconds

	int WiFi;

	int SystemMode; // 0 -> Auto (with attitudeplaning activated), 1 -> Manual (with attitudeplaning deactivated)
...
struct ImuDatamsg{
	...
	double orientation[3];  // actual orientation of the FLOATSAT in term of Pitch Roll Yaw
...
extern void matrix_mult(double mat_1[][3], double mat_2[][2],int R1,int C1,int C2,double *Result);
