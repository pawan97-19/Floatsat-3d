...
public:
	 ...
	 * @return
	 */
	int init(const char *_ssid, const char *_pw, bool _accessPoint=false);
	 ...
	 */
	void enableUDPConnection(const char *_udp_destination, uint32_t _udp_port);


	void enableTCPConnection(const char *_tcp_destination, uint32_t _tcp_port);
	...
	class UpdateThread : public Thread {
		...
		Fifo<UDPMsg,5> *txFifo;
	public:
		UpdateThread(ESP8266 *_esp8266, Fifo<UDPMsg,5> *txFifo);
		void run();
	} updatethread;
};
