...
uint64_t thread_period = 30; // milliseconds
...
private:
	...
	uint64_t thread_period;

public:

	sPID_Data PID_MotorSpeed0;
	sPID_Data PID_MotorSpeed1;
	sPID_Data PID_MotorSpeed2;

	Attitudecontrol(const char* name,sPID_Data PID_control0,sPID_Data PID_control1,sPID_Data PID_control2,uint64_t thread_period) : Thread(name) {
		this->PID_MotorSpeed0 = PID_control0;
		this->PID_MotorSpeed1 = PID_control1;
		this->PID_MotorSpeed2 = PID_control2;
	...
	{
		*(this->MotorSpeedError + 0) = *(ref_DCspeed + 0) - *(sens_DCspeed + 0);
		*(this->MotorSpeedError + 1) = *(ref_DCspeed + 1) - *(sens_DCspeed + 1);
		*(this->MotorSpeedError + 2) = *(ref_DCspeed + 2) - *(sens_DCspeed + 2);

		PID(&this->PID_MotorSpeed0, *(this->MotorSpeedError + 0));
		PID(&this->PID_MotorSpeed1, *(this->MotorSpeedError + 1));
		PID(&this->PID_MotorSpeed2, *(this->MotorSpeedError + 2));
		...
		if (*(ref_DCspeed + 0) == 0 && *(sens_DCspeed + 0) == 0){
			...
			MotorSet(this->PID_MotorSpeed0.Usat,0);
		}
		...
		if (*(ref_DCspeed + 1) == 0 && *(sens_DCspeed + 1) == 0){
			...
			MotorSet(this->PID_MotorSpeed1.Usat,1);
		}
		...
		if (*(ref_DCspeed + 2) == 0 && *(sens_DCspeed + 2) == 0){
			...
			MotorSet(this->PID_MotorSpeed2.Usat,2);
		}
	}
	...
	void run() {
		...
		for(int i = 0;i<3;i++){
			TelecommandDataReceiver2.Is[i] = 0.0062;
		}

		for(int i = 0;i<3;i++){
			TelecommandDataReceiver2.Irw[i] = 0.0011;
		}

		for(int i = 0;i<3;i++){
			nextMove.euler[i] = 0.0;
		}

		for(int i = 0;i<3;i++){
			ImuDataReceiver2.acceleration[i] = 0.0;
		}
		...
		TelecommandDataReceiver2.ThreadPeriod[3] = 80;
		...
		while (1) {
			...
			for(int i = 0;i<3;i++){
				this->MotorSpeedError[i] = TelecommandDataReceiver2.DCSpeed[i] - ACmsg.DCSpeed[i];
			}
			...
			if (ACmsg.DCSpeed[0]==0 && TelecommandDataReceiver2.DCSpeed[0]==0)
			{
				Motor_PWM_X.write(0);
				Motor_IN_X1.setPins(0);
				Motor_IN_X2.setPins(0);
			...
			if (ACmsg.DCSpeed[1]==0 && TelecommandDataReceiver2.DCSpeed[1]==0)
			{
				Motor_PWM_Y.write(0);
				Motor_IN_Y1.setPins(0);
				Motor_IN_Y2.setPins(0);
			...
			if (ACmsg.DCSpeed[2]==0 && TelecommandDataReceiver2.DCSpeed[2]==0)
			{
				Motor_PWM_Z.write(0);
				Motor_IN_Z1.setPins(0);
				Motor_IN_Z2.setPins(0);
			...
			this->thread_period = TelecommandDataReceiver2.ThreadPeriod[3] == -1? this->thread_period:TelecommandDataReceiver2.ThreadPeriod[3];
			deltaTime = timeNow;
			AC2TelemetrieTopic.publish(ACmsg);
			suspendCallerUntil(NOW() + this->thread_period*MILLISECONDS);
		}
	}
};

Attitudecontrol Attitudecontrol("Attitudecontrol",PID_MotorSpeed,PID_MotorSpeed1,PID_MotorSpeed2,thread_period);
